...
-   [ ] Start your project using (replace `project_name` with your project name and remove the curly braces):
    ```
    django-admin startproject insuranceai --extension py,json,yml,yaml,toml --name Dockerfile,README.md,.env.example,.gitignore,Makefile --template=https://github.com/vintasoftware/django-react-boilerplate/archive/refs/heads/main.zip
    ```
    ...
    django-admin startproject insuranceai . --extension py,json,yml,yaml,toml --name Dockerfile,README.md,.env.example,.gitignore,Makefile --template=https://github.com/vintasoftware/django-react-boilerplate/archive/refs/heads/main.zip
    ```
In the next steps, always remember to replace insuranceai with your project's name (in case it isn't yet):
-   [ ] Above: don't forget the `--extension` and `--name` params!
-   [ ] Go into project's root directory: `cd insuranceai`
-   [ ] Change the first line of README to the name of the project
-   [ ] Add an email address to the `ADMINS` settings variable in `insuranceai/backend/insuranceai/settings/base.py`
-   [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `insuranceai/backend/insuranceai/settings/production.py`
...
-   Do the following:
    -   Create a git-untracked `local.py` settings file:
        `cp backend/insuranceai/settings/local.py.example backend/insuranceai/settings/local.py`
    -   Create a git-untracked `.env.example` file:
        `cp backend/.env.example backend/.env`

### If you are using Docker:

-   Open the `backend/.env` file on a text editor and uncomment the line `DATABASE_URL=postgres://insuranceai:password@db:5432/insuranceai`
...
-   Open the `backend/.env` file on a text editor and do one of the following:
    ...
    -   If you wish to use PostgreSQL locally, uncomment and edit the line `DATABASE_URL=postgres://insuranceai:password@db:5432/insuranceai` in order to make it correctly point to your database URL
...
-   `poetry run celery --app=insuranceai worker --loglevel=info`
...
Chances are your project name isn't unique in Render, and you'll get a randomized suffix as your full app URL like: `https://insuranceai-a1b2.onrender.com`.

But this will only happen after the first deploy, so you are not able to properly fill `ALLOWED_HOSTS` yet. Simply set it to `*` then fix it later to something like `insuranceai-a1b2.onrender.com` and your domain name like `example.org`.
